...
# =========================================================================
# 【智能环境自适应模块 & 纯净版工具箱】
# 彻底移除对官方 utils 文件夹和 CUDA 算子的依赖，实现单文件独立运行！
# =========================================================================
def print_log(msg, logger=None):
    """替换官方的复杂日志工具"""
    print(f"[{logger if logger else 'LOG'}] {msg}")

def fps(xyz, npoint):
    """纯 PyTorch 实现的最远点采样 (FPS)，替代官方的 misc.fps"""
    device = xyz.device
    B, N, C = xyz.shape
    ...
    gathered_points = torch.zeros(B, npoint, 3, device=device)
    for b in range(B):
        gathered_points[b] = xyz[b, centroids[b]]
    return gathered_points

try:
    from knn_cuda import KNN
    from extensions.chamfer_dist import ChamferDistanceL1, ChamferDistanceL2
    print_log("成功加载底层 CUDA 加速算子 (KNN, Chamfer Distance)", "System")
except ImportError:
    print_log("自动切换为纯 PyTorch 兼容模式 (支持 CPU/GPU 无缝运行)", "System")
    class KNN(nn.Module):
        ...
        def forward(self, ref, query):
            ...
            knn_dist, knn_idx = torch.topk(dist, self.k, dim=-1, largest=False)
            return knn_dist, knn_idx

    class ChamferDistanceL2(nn.Module):
        def forward(self, p1, p2):
            dist = torch.cdist(p1, p2) ** 2
...
# 取消了 @MODELS.register_module() 装饰器，直接定义核心类
class Point_MAE(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log('Initializing...', logger ='Point_MAE')
        self.config = config
        self.trans_dim = config.transformer_config.trans_dim
